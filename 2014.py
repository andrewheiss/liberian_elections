...
def parse_precinct(url, house_district, voters, place_name, turnout):
    ...
    if len(actual_results) > 1:
        ...
        # Presidential data
        pres_data_raw = extract_data(actual_results[0])
        pres_data_long = convert_to_long(pres_data_raw, "Senate", metadata)
    ...
    for key, value in final_results.items():
        w = csv.DictWriter(open('2014.csv', 'a'), fieldnames)
...
def extract_data(table):
    ...
    for row in table.findAll('tr'):
        ...
        vote_cols_raw = [x.contents for x in results[1:]]
        vote_cols = []
        for col in vote_cols_raw:
            if not col:
                col = ['0']
            vote_cols.append(col[0])
        ...
        if re.search("valid|total", cand_name_party, re.I | re.M):
            if cand_name_party == "Total Valid":
                valid = [votes.strip() for votes in vote_cols]
                clean_results['valid'] = valid

            if cand_name_party == "Invalid":
                invalid = [votes.strip() for votes in vote_cols]
                clean_results['invalid'] = invalid

            if cand_name_party == "Total":
                total = [votes.strip() for votes in vote_cols]
        ...
        # Save candidate information as dict to cand_list
            ...
            cand_votes = [votes.strip() for votes in vote_cols]
...
def get_county_list():
    ...
    # Parse HTML
    soup = BeautifulSoup(get_url(base_url))
    logging.info("Getting the list of all the "
                 "counties from {0}".format(base_url))

    # All the counties links are in area tags in an image map
    county_tags = soup.findAll('area')
    counties = [base_url + tag['href'] for tag in county_tags]
    return(counties)
...
def parse_county(url):
    # Global again. ¯\_(ツ)_/¯
    global base_url

    soup = BeautifulSoup(get_url(url))

    # There are 2 <h2>s on the page: the county name and turnout numbers for
    # the senate election.
    h2s = soup.findAll('h2')
    county_name = h2s[0].contents[0].replace(' County', '')
    turnout_raw = h2s[1]

    logging.info("Parsing information for {0} "
                 "county from {1}".format(county_name, url))
    ...
    # Save to dictionary
    turnout = {}
    turnout['county'] = county_name
    turnout['first_round'] = turnout_clean[0]
    turnout['first_round_link'] = polling_places[0]
...
def parse_county_precincts(county):
    ...
    soup = BeautifulSoup(get_url(county['first_round_link']))
...
# --------------
# Set up log
logging.basicConfig(filename='2014.log', filemode='a', level=logging.DEBUG,
                    format='%(levelname)s %(asctime)s: %(message)s')
logging.captureWarnings(True)

# GLobal variables
base_url = "http://www.necliberia.org/senate2014/"
csv_started = False
